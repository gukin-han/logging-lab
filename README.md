...
운영 환경에서 `jdbc.resultset` 로그가 **1시간에 34GB(2.7억 건)** 쏟아지며 시스템이 마비된 실제 장애를 재현하고,
로깅 설정 변경이 TPS에 미치는 영향을 정량적으로 측정한 실험이다.

---

## 실험 배경

개발 환경용 로깅 설정(`jdbc.resultset=DEBUG`)이 운영 환경에 그대로 배포되면서,
**코드 변경 없이 로그만으로 시스템이 마비**되는 장애가 발생했다.
