...
운영 환경에서 로컬/개발용 로깅 설정이 그대로 배포되면서
**jdbc.resultset 쿼리 덤프 로그**가 1시간에 2.7억 건(34GB)씩 발생하는 장애가 있었다.

CloudWatch Logs Insights 분석 결과, 도메인 INFO 로그는 원인이 아니었고
jdbc.resultset 레벨의 과도한 출력이 근본 원인이었다.
