...
실험 전 가설은 "대량의 로그 String 객체가 힙 Eden 영역을 채우고, Minor GC가 초당 수회 발생하며 Stop-The-World를 유발한다"였다.

**실제 결과는 달랐다:**

- **Phase 2 (Sync):** GC 활동 거의 없음. CPU 사용률도 낮음.
- **Phase 4 (Async):** GC 활동 0.1~0.3% 수준으로 미미. 힙 톱니 패턴 없음.
- **Phase 5 (jdbc OFF):** GC 활동 없음. 오히려 CPU 사용률이 Phase 2보다 ~20% 높음.

### 왜 GC가 아닌가

**Phase 2 (Sync):** 동기 로깅이라 스레드가 콘솔에 로그를 다 쓸 때까지 블로킹된다. 블로킹 중에는 새 요청을 처리하지 못하므로 객체 생성 자체가 일어나지 않는다. TPS가 14밖에 안 되니 힙에 부담을 줄 만큼의 객체가 만들어지지 않은 것이다.

**Phase 4 (Async):** 비동기라 메인 스레드는 블로킹되지 않지만, Disruptor 큐가 금방 가득 차면서 결국 back-pressure가 발생한다. TPS가 18 수준이라 객체 생성률 자체가 GC를 자극할 만큼 높지 않았다.

**Phase 5 (jdbc OFF):** TPS 2,429로 초당 2,000건 이상을 실제로 처리하므로 CPU를 더 많이 사용하는 것이 당연하다. Phase 2는 스레드가 I/O 대기 상태라 CPU를 오히려 덜 쓰고 있었다.

### 진짜 병목: 순수 I/O 블로킹

이 실험에서 성능을 죽인 것은 GC가 아니라 **콘솔 출력(SYSTEM_OUT)의 동기 I/O 블로킹**이다. 비즈니스 스레드가 로그를 콘솔에 쓰는 동안 완전히 멈추면서 TPS가 170배 폭락한 것이다.
...
2. **병목은 GC가 아니라 I/O다.** VisualVM 프로파일링 결과, 힙 메모리나 GC는 문제가 아니었다. 동기 콘솔 출력이 스레드를 블로킹하여 요청 처리 자체를 멈추게 한 것이 근본 원인이다.

3. **Sync/Async, Text/JSON은 극단적 상황에서만 의미 있다.** 로그량이 적정하면 차이가 미미하고, 로그량이 극단적이면 어떤 최적화도 한계가 있다.

4. **원천 차단이 유일한 해결책이다.** Phase 5에서 jdbc.resultset을 OFF 하자 TPS가 97.7% 회복. 비동기 로깅은 보험이지 해결책이 아니다.
